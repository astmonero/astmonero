...
#if defined(NDEBUG)
# error "AstmoneroCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x909884bf9643e40fb25906c979a4bedb1f033ffb49d9d102901f6de3dbacfe68");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AstmoneroCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AstmoneroCoin Signed Message:\n";
...
{
    // AstmoneroCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 6 * 60 * 60;; // AstmoneroCoin: 1 hour
static const int64 nTargetSpacing = 2 * 60; // AstmoneroCoin: 5 Minutes.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AstmoneroCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // AstmoneroCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x909884bf9643e40fb25906c979a4bedb1f033ffb49d9d102901f6de3dbacfe68");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is AstmoneroCoin 102018 ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("260684710fa689ad5023690c80f3a99c8f13f8d45b8c857fbbbi9bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;	
        block.nTime    = 1515596092;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1583137;
        ...
        assert(block.hashMerkleRoot == uint256("0x723c5311cc518691d14bad0f3e7e9ddbedc1c966433bd00ba6b4d1870ef237f2"));
        ...
        if (true && block.GetHash() != hashGenesisBlock)
{
    ...
    uint256 thash;
    char scratchpad[SCRYPT_SCRASTMHPAD_SIZE];
...
unsigned char pchMessageStart[4] = { 0xc2, 0xd6, 0xbf, 0xbd }; // AstmoneroCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AstmoneroCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AstmoneroCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AstmoneroCoinMiner(CWallet *pwallet)
{
    printf("AstmoneroCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("astmonerocoin-miner");
    ...
    try { loop {
        ...
        printf("Running AstmoneroCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AstmoneroCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AstmoneroCoinMiner, pwallet));
}
