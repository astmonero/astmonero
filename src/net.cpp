...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "AstmoneroCoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"132.148.148.250", "132,148.148.250"},
     {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. AstmoneroCoin is probably already running."), addrBind.ToString().c_str());
        else
